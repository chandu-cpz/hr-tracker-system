  ...
  const submitUser = async (e) => {
    ...
    if (validateForm()) {
      try {
        ...
        // Assuming the loginUser action dispatches an async request to the backend

        // If you reach here, the login was successful
       
      } catch (error) {
        // If there's an error, handle it and update the UI accordingly
            ...
            <div className="tw-flex tw-flex-col tw-items-start md:tw-flex-col">
              ...
              <div className="tw-mb-4 tw-flex tw-w-full tw-flex-col md:tw-mb-0 md:tw-mr-4">
                ...
                <input
                  ...
                  onChange={handleChange}
                />
                <span className="tw-text-red-500">{errors.password}</span>
              </div>
              <button
                type="submit"
